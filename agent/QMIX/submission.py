...
class RLAgent(object):
    ...
    def choose_action(self, obs):
        ...
        mask = torch.zeros((obs.shape[0], 4))

        # mask[:, 0] = (obs[:, 2 + 5*5] + obs[:, 3 + 5*5] == 1).float()
        # mask[:, 2] = (obs[:, 2 + 7*5] + obs[:, 3 + 7*5] == 1).float()
        # mask[:, 3] = (obs[:, 2 + 8*5] + obs[:, 3 + 8*5] == 1).float()
        # mask[:, 1] = (obs[:, 2 + 10*5] + obs[:, 3 + 10*5] == 1).float()
