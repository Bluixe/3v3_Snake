'''
    utils funtions for greedy-loop algorithm
    this codes are adapted from the project of snakes1v1 by Wenhao Zhang, Haoyu Zhen, Qiping Cui, 
    while Wenhao Zhang is also the member of this project.
    the original project can be found at https://github.com/Bluixe/snake1v1_group8
'''
...
def greedy_main(states):
    '''
        main function for greedy-loop algorithm
        :param states: raw states
    '''
    actions = []
    for state in states:
...
def greedy(state_map, beans, snakes, width, height, ctrl_agent_index):
...
def defense(state_map, beans, snakes, width, height, ctrl_agent_index):
...
def compre_greedy_defense(state_map, beans, snakes, width, height, ctrl_agent_index):
    ...
    if len(snakes[my_index])>limitation:
        actions=defense(state_map, beans, snakes, width, height, ctrl_agent_index)
    else:
        actions=greedy(state_map, beans, snakes, width, height, ctrl_agent_index)
    return actions
