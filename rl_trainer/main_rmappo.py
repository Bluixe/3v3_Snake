'''
    trainning code for recurrent mappo
    modified from main_mappo.py
'''
        ...
        while step < self.args.episode_length:
            ...
            if done:
                if np.sum(episode_reward[:3]) > np.sum(episode_reward[3:]):
                    step_reward = get_dense_reward(info, self.ctrl_agent_index, reward, beans, score=1)
                elif np.sum(episode_reward[:3]) < np.sum(episode_reward[3:]):
                    step_reward = get_dense_reward(info, self.ctrl_agent_index, reward, beans, score=2)
                else:
                    step_reward = get_dense_reward(info, self.ctrl_agent_index, reward, beans, score=0)
            else:
                if np.sum(episode_reward[:3]) > np.sum(episode_reward[3:]):
                    step_reward = get_dense_reward(info, self.ctrl_agent_index, reward, beans, score=3)
                elif np.sum(episode_reward[:3]) < np.sum(episode_reward[3:]):
...
def main(args):
    ...
    def unpack(res):
        '''
            unpack the data from the rollout workers
            return episode-wise data
        '''
        obs_ep, action_ep, reward_ep, next_obs_ep, done_ep, global_state_ep, episode_reward = [], [], [], [], [], [], []
        for i in range(len(res)):
            obs_ep.append(res[i][0])
            action_ep.append(res[i][1])
            reward_ep.append(res[i][2])
            next_obs_ep.append(res[i][3])
            done_ep.append(res[i][4])
            global_state_ep.append(res[i][5])
            episode_reward.append(res[i][6])
        obs_ep = np.stack(obs_ep)
        action_ep = np.stack(action_ep)
        reward_ep = np.stack(reward_ep)
        next_obs_ep = np.stack(next_obs_ep)
        done_ep = np.stack(done_ep)
        global_state_ep = np.stack(global_state_ep)
        episode_reward = np.stack(episode_reward)
        return obs_ep, action_ep, reward_ep, next_obs_ep, done_ep, global_state_ep, episode_reward
    ...
    while episode < args.max_episodes:
        ...
        obs_ep, action_ep, reward_ep, next_obs_ep, done_ep, global_state_ep, episode_reward = unpack(res)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--a_lr', default=5e-5, type=float)
    parser.add_argument('--c_lr', default=5e-5, type=float)
    ...
    parser.add_argument('--sil', default=0, type=bool)
