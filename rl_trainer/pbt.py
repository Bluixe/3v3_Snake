'''
    training code for population based trainning using mappo algorithm
'''
...
class RolloutWorker:
    ...
    def __init__(self, args):
        ...
        self.obs_dim = 122
        self.model1 = MAPPO(self.obs_dim, self.act_dim, self.ctrl_agent_num, args)
        self.model2 = MAPPO(self.obs_dim, self.act_dim, self.ctrl_agent_num, args)
        self.eps = 0
    ...
    def collect_data(self):
        ...
        while step < self.args.episode_length:
            ...
            logits = self.model1.greedy_init(obs, state[:3], self.eps)

            logits_opp = self.model2.greedy_init(obs_opp, state[3:], self.eps)
        ...
        # actor_loss, critic_loss, mean_reward = self.model.update((obs_ep, action_ep, reward_ep, next_obs_ep, done_ep, global_state_ep))
    ...
    def update_model(self, model1, model2):
        self.model1.actor.load_state_dict(model1.actor.state_dict())
        self.model1.critic.load_state_dict(model1.critic.state_dict())
        self.model2.actor.load_state_dict(model2.actor.state_dict())
        self.model2.critic.load_state_dict(model2.critic.state_dict())
        self.eps += 1
...
def main(args):
    ...
    '''
        define each agent's 
    '''
    models = [MAPPO(obs_dim, act_dim, ctrl_agent_num, args) for _ in range(args.num_agents)]
    agent_pair = list(itertools.product(range(args.num_agents), range(args.num_agents)))
    ...
    while episode < args.max_episodes:
        ...
        score = [[] for _ in range(args.num_agents)]

        '''
            exploration phase
        '''

        for _ in range(args.mutation_interval):
            agent_pair = random.shuffle(agent_pair)
            for idx1, idx2 in agent_pair:
                '''
                    assign state dict for each rollout worker
                '''

                [w.update_model(models[idx1], models[idx2]) for w in workers]
                ...
                actor_loss, critic_loss, mean_reward = models[idx1].update((obs_ep, action_ep, reward_ep, next_obs_ep, done_ep, global_state_ep))
        ...
        '''
            mutation phase
            update the worst agent with the best agent's state dict
        '''
        mean_score = np.array([np.mean(s) for s in score])
        best_agent = np.argmax(mean_score)
        worst_agent = np.argmin(mean_score)


        models[worst_agent].actor.load_state_dict(models[best_agent].actor.state_dict())
        models[worst_agent].critic.load_state_dict(models[best_agent].critic.state_dict())


        if episode % 100 == 0:
            models[best_agent].save_model(run_dir, episode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mutation_interval', default=10, type=int)
